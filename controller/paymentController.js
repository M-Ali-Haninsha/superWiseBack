    ...
    instance.orders.create(options, async (err, order) => {
      ...
      if (order) {
        ...
        await clientModel.updateOne(
          { _id: userId, "payment.workerId": req.body.workerId },
          {
            $set: {
              "payment.$.status": "completed",
              "payment.$.payedDate": Date.now(),
            },
          },
          { upsert: true }
        );
        await clientModel.updateOne(
          { _id: userId, "workStatus.workerId": req.body.workerId },
          { $set: { "workStatus.$.paymentStatus": "completed" } },
          { upsert: true }
        );
